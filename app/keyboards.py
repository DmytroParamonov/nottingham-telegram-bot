...
def private_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🛒 Торг на ринку", callback_data="menu:market")],
        [InlineKeyboardButton(text="🛞 Мій вантаж", callback_data="menu:bag")],
        [InlineKeyboardButton(text="📦 Мої товари", callback_data="menu:hand")],
        [InlineKeyboardButton(text="🤝 Угоди з вартою", callback_data="menu:deals")],
...
def market_keyboard(hand: list[str], selected: list[str]) -> InlineKeyboardMarkup:
    ...
    rows.append([
        ...
        InlineKeyboardButton(text="✅ Завершити торг", callback_data="market:confirm"),
    ])
...
def bag_keyboard(hand: list[str], bag: list[str]) -> InlineKeyboardMarkup:
    ...
    rows.append([
        ...
        InlineKeyboardButton(text="🔒 Запечатати вантаж", callback_data="bag:lock"),
    ])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def bribe_keyboard(current: int = 0) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🤝 Запропонувати варті", callback_data=f"bribe:confirm:{current}")],
    ])
...
def sheriff_keyboard(merchants: list[dict], bribes: dict[int, int]) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=rows or [[InlineKeyboardButton(text="✅ Усі повозки вирішено", callback_data="noop")]]
    )
...
def sheriff_decision_keyboard(merchant_id: int, bribe: int = 0) -> InlineKeyboardMarkup:
    ...
    if bribe > 0:
        rows.append([
            InlineKeyboardButton(
                text=f"🪙 Взяти {bribe} крон і пропустити",
    ...
    rows.extend([
        ...
        [InlineKeyboardButton(text="🔍 ОБШУКАТИ ПОВОЗКУ", callback_data=f"sheriff:inspect:{merchant_id}")],
        [InlineKeyboardButton(text="⬅️ До купців", callback_data="sheriff:list")],
    ])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def deal_builder_keyboard(merchants: list[dict], deal: dict) -> InlineKeyboardMarkup:
    ...
    for merchant in merchants:
        ...
        rows.append([
            ...
            InlineKeyboardButton(
                text=f"{inspect_mark} Обшукати {short}",
    ...
    rows.append([
        InlineKeyboardButton(text="🏪 Товари з лавки", callback_data="deal:goods:stand"),
        InlineKeyboardButton(text="🛞 Товари з вантажу", callback_data="deal:goods:bag"),
    ])
    follow = "📌 Додатковий обшук"
    ...
    rows.append([
        ...
        InlineKeyboardButton(text="📤 Надіслати варті", callback_data="deal:offer"),
    ])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def deal_followup_keyboard(merchants: list[dict], deal: dict) -> InlineKeyboardMarkup:
    rows: list[list[InlineKeyboardButton]] = [
        [InlineKeyboardButton(text="🚫 Без додаткового обшуку", callback_data="deal:followup:set:0")]
    ]
